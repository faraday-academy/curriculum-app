<template>
  <v-toolbar color="info">
    ...
    <v-toolbar-items v-if="$vuetify.breakpoint.xsOnly">
      <v-btn @click="openDrawer()"> Menu </v-btn>
    </v-toolbar-items>

    <v-toolbar-items v-else>
      <v-btn to="/" exact text> Home </v-btn>
      <v-btn to="/curricula" exact text> View All </v-btn>
      <v-btn v-if="user.token" to="/curricula/create" text> Create </v-btn>
      <v-btn
        v-if="!user.token"
        to="/login"
        color="secondary"
